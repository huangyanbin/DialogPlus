package com.bin.david.dialoglib;
...
import com.bin.david.dialoglib.anim.IBaseDialogAnim;
...
public class BaseDialog extends Dialog {
	...
	private BaseDialog(Builder builder) {
		...
		if(builder.contentView != null){
			...
			if(builder.contentViewBg != INVALID){
				builder.contentView.setBackgroundResource(builder.contentViewBg);
			}
		}
		...
		if(builder != null) {
			if (builder.context != null && builder.context instanceof Activity) {
				...
				if(!activity.isFinishing()) {
					if(builder.anim != null){
						builder.anim.show(this,builder.contentView);
					}else{
						showDialog();
					}
				}
	...
	public void dismiss() {
		if(builder != null) {
			if (builder.context != null && builder.context instanceof Activity) {
				...
				if (!activity.isFinishing()) {
					if(builder.anim != null){
						builder.anim.dismiss(this,builder.contentView);
					}else{
						super.dismiss();
					}
				}
	...
	public static class Builder {
		...
		public Builder setDialogAnimtor(IBaseDialogAnim anim) {
			this.anim = anim;
			return this;
		}


		public Builder setContentViewBackground(int contentViewBg) {
